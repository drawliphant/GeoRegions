...
void RegionTester::testCreateFromStream()
{
    ...
    // Load a file with the and four nations
    {
        ...
        {//I cannot for the life of me figure out why this fails
            //I try to write my own and it works
...
void RegionTester::testSubRegions()
{
    ...
    std::string loader="2,aNation,900,800";
    std::string sub="3,aState,80,70";
    std::string lower="4,aCounty,7,6";
    Region *region = Region::create(loader);
    Region *aNation=Region::create(sub);
    Region *aState=Region::create(lower);
    region->addSubregion(aNation);
    aNation->addSubregion(aState);
    aNation->addSubregion(Region::create("4,2County,36,400"));
    aNation->addSubregion(Region::create("4,threeCounty,35,399"));
    if(aNation->getSubRegionCount()!=3){
        std::cout<<"Nation didnt have 3 subs, had "<<aNation->getSubRegionCount()<<std::endl;
    }
    if(region->getSubRegionCount()!=1){
        std::cout<<"Nation didnt have 3 subs, had "<<aNation->getSubRegionCount()<<std::endl;
    }
...
void RegionTester::testComputeTotalPopulation()
{
    ...
    std::string nat="2,aNation,900,800";
    std::string stat="3,aState,80,70";
    std::string cat="4,aCounty,7,6";
    Region *nation = Region::create(nat);
    Region *state=Region::create(stat);
    Region *county=Region::create(cat);
    state->addSubregion(county);
    nation->addSubregion(state);

    if(nation->computeTotalPopulation()!=987){
        std::cout<<"Nation did not have population of 987, had "<<nation->computeTotalPopulation()<<std::endl;
    }
    if(state->computeTotalPopulation()!=87){
        std::cout<<"State did not have population of 87, had "<<state->computeTotalPopulation()<<std::endl;
    }
    if(county->computeTotalPopulation()!=7){
        std::cout<<"County did not have population of 7, had "<<county->computeTotalPopulation()<<std::endl;
    }
